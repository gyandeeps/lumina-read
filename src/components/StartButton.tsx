...
export const StartButton: React.FC<StartButtonProps> = ({
  ...
  if (status === 'idle') {
    return (
      <button
        onClick={onInit}
        className="group relative px-10 py-6 text-2xl font-bold font-kids text-white bg-gradient-to-r from-teal-500 to-cyan-500 rounded-3xl shadow-xl transition-all duration-300 transform hover:scale-105 active:scale-95 hover:shadow-teal-300 hover:shadow-2xl border-4 border-white flex items-center gap-3 animate-bounce-subtle"
      >
        <Wand2 className="w-8 h-8 group-hover:rotate-12 transition-transform duration-300 text-amber-300" />
        <span>Tap to Get Ready!</span>
        <span className="absolute -top-3 -right-3 px-3 py-1 bg-yellow-400 text-xs font-sans text-teal-950 rounded-full font-bold rotate-12 uppercase tracking-wide border-2 border-white shadow">
          Local AI
        </span>
      </button>
    );
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col items-center gap-4 bg-white/70 backdrop-blur-md p-8 rounded-3xl border border-white/60 shadow-lg max-w-sm w-full mx-auto border-teal-100 shadow-teal-100/10">
        ...
        <div className="w-full bg-teal-50 rounded-full h-4 overflow-hidden border border-teal-100">
          <div
