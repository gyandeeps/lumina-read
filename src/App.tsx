...
import { BookOpen, Sparkles, Volume2, Loader2, CheckCircle, BookOpenCheck, Filter } from 'lucide-react';
...
function App() {
  ...
  const filteredStories = difficultyFilter === 'all'
    ? stories
    : stories.filter((s) => s.difficulty === difficultyFilter);

  if (selectedStory) {
    return (
      <main className="min-h-screen bg-pink-50/50 py-4 page-enter">
        <ReadingScreen
          story={selectedStory}
          onBack={handleBack}
      ...
      {showResumePrompt && resumeStory && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-fade-in-up">
          <div className="glass-card max-w-md w-full p-8 rounded-3xl text-center shadow-2xl">
            <BookOpenCheck className="w-14 h-14 text-pink-500 mx-auto mb-4" />
            <h3 className="font-kids text-2xl font-bold text-slate-800 mb-2">
              Welcome Back! 📖
            </h3>
            <p className="text-slate-500 font-sans text-sm mb-6">
              You were reading <span className="font-bold text-pink-500">"{resumeStory.title}"</span> — sentence {resumeStory.sentenceIndex + 1}. Pick up where you left off?
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 text-sm font-bold font-kids text-white bg-gradient-to-r from-pink-500 to-rose-500 rounded-2xl shadow-md hover:shadow-lg transition-all transform hover:scale-105 active:scale-95"
              >
                Continue! ✨
              </button>
            </div>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          {/* Difficulty Badge */}
                          <span className={`text-xs font-sans font-bold px-3 py-1 rounded-full border ${diffConfig.color}`}>
                            {diffConfig.label}
                          </span>
                          {/* Completed Checkmark */}
                          {isCompleted && (
